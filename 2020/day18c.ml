type 'a parser = string * int -> ('a * string * int) option

let return x (str, off) = Some (x, (str, off))
...
let char c (str, off) = try if str.[off] = c then Some (c, (str, off + 1)) else None with _ -> None
...
let _ = 
    "day18.input" |> open_in
    |> seq_hylo
        (fun cin -> try Some (cin |> input_line, cin) with _ -> None)
        (fun (m, n) s -> m + fst (Option.get (expr1 (s, 0))), n + fst (Option.get (expr2 (s, 0))))
        (0, 0)
