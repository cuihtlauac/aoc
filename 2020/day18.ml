type 'a parser = char Seq.t -> ('a * char Seq.t) option
...
let char c str = match str () with Seq.Cons (x, str) when x = c -> Some (x, str) | _ -> None
...
let _ = 
    "day18.input" |> open_in
    |> seq_hylo
        (fun cin -> try Some (cin |> input_line |> String.to_seq, cin) with _ -> None)
